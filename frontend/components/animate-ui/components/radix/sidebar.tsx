...
function Separator({
  ...
  return (
    <div
      role="separator"
      className={cnUtil('shrink-0 bg-border h-px w-full', className)}
      {...props}
    />
  );
}
...
function Skeleton({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cnUtil('animate-pulse rounded-md bg-muted', className)}
      {...props}
    />
  );
}
...
function SidebarInset({ className, ...props }: SidebarInsetProps) {
  return (
    <main
      ...
      className={cn(
        'bg-background relative flex w-full min-w-0 flex-1 flex-col overflow-auto',
...
function SidebarContent({ className, scrollable = true, ...props }: SidebarContentProps) {
  return (
    <div
      ...
      className={cn(
        ...
        scrollable
          ? 'no-scrollbar overflow-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/15 [&::-webkit-scrollbar-thumb:hover]:bg-white/25'
          : 'overflow-hidden',
        className,
      )}
      {...props}
...
function SidebarMenuButton({
  ...
  let tooltipContent: React.ComponentProps<typeof TooltipContent> | undefined;
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        // Increases the hit area of the button on mobile.
        'z-[1] cursor-pointer text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground peer-hover/menu-button:text-sidebar-accent-foreground absolute top-1.5 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        'after:absolute after:-inset-2 md:after:hidden',
