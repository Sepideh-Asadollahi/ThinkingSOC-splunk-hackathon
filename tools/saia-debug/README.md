...
Small tool **outside ThinkingSOC** to compare Splunk AI Assistant paths (UI `/predict` vs MCP `generatespl`).

**ThinkingSOC production SPL** uses REST **`/predict`** + MCP **`splunk_run_query`** — see [docs/13-cim-investigation-spl-mcp.md](../../docs/13-cim-investigation-spl-mcp.md).

## Common cause (summary)

| Path | Splunk endpoint | Typical |
|------|-----------------|---------|
| **UI chat / ThinkingSOC** | `POST .../Splunk_AI_Assistant_Cloud/predict` | Works |
...
**ThinkingSOC backend** auto-repairs SAIA `cloud_connected_configurations` when KV/conf is incomplete (`TSOC_SAIA_AUTO_REPAIR=true`, default). See [docs/13-cim-investigation-spl-mcp.md](../../docs/13-cim-investigation-spl-mcp.md#saia-cloud-config-auto-repair).
...
Final **Diagnosis** section explains UI v1 vs MCP v2 — not a ThinkingSOC application bug.
