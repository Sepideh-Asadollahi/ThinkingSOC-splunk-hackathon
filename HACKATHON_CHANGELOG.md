...
## Added for the hackathon — ThinkingSOC Forge
...
- Forge panel in the Security Investigation Overview with evidence, errors, approval, reuse, and honest status labels.
- Dedicated Runbooks Sidebar section and NeonGlass Forge Settings page with feature flag, step cap, manual baseline, artifact limit, dependency readiness, and fixed trust-policy display.
...
- Corrected the internal storage query cap so the documented Forge artifact scan range (`50..1000`) is effective.
- Reframed the root README around the hackathon Forge delta and added public product/demo plus U.S. SOC economic-impact documents with sourced, reproducible capacity and ROI formulas.
