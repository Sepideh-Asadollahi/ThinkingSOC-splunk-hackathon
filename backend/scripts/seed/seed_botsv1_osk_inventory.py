#!/usr/bin/env python3
"""Seed PostgreSQL inventory for botsv1 osk.exe Sysmon webhook sample enrichment."""
...
_DEFAULT_SAMPLE = _REPO / "scripts" / "samples" / "splunk-webhook-botsv1-osk-sysmon.json"
_DEMO_DIR = Path(__file__).resolve().parents[2] / "data" / "demo" / "botsv1_osk_sysmon"
...
from services.inventory.assets import create_asset, update_asset
...
from services.inventory.users import create_user, update_user
...
async def _verify_enrichment(settings, normalized: dict) -> None:
    ...
    if out.resolved_asset_id != "botsv1-we8105desk":
        raise SystemExit("expected resolved_asset_id=botsv1-we8105desk, got {0}".format(out.resolved_asset_id))
    if out.resolved_user_id != "SYSTEM":
        raise SystemExit("expected resolved_user_id=SYSTEM, got {0}".format(out.resolved_user_id))
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Webhook JSON used to verify enrichment (default: botsv1 osk Sysmon sample)",
    )
    ...
    print("sample host={0!r} user={1!r}".format(normalized.get("host"), normalized.get("user")))
    ...
    from services.inventory.assets import get_asset
    from services.inventory.users import get_user

    asset = await get_asset(settings, "botsv1-we8105desk")
    user = await get_user(settings, "SYSTEM")
    print("asset:", asset.model_dump())
    print("user:", user.model_dump())
    print("ok: enrichment matches botsv1-we8105desk + SYSTEM")
