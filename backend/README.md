...
- `GET /health`
- `POST /api/v1/alerts/splunk-ingest` — JSON handoff from Splunk alert action (Bearer if `TSOC_INGEST_TOKEN` set). Returns `202` when `TSOC_INGEST_AUTO_ANALYZE=true` (default), else `200`. **Configuration is not overridable via URL query parameters.**
